...
/**
 示例
 String yourFileName = "123";//不用命名后缀名
 ...
 .setDestinationDirectoryPath(Environment.getExternalStoragePublicDirectory(
    Environment.DIRECTORY_PICTURES).getAbsolutePath())//设置存储路径
 .build()
...
public class CompressHelper {
    ...
    public File compressToFile(File file) {
        return CompressUtil.compressImage(context, Uri.fromFile(file), maxWidth, maxHeight,
     ...
     * @return      压缩后的文件
     */
    public File compressToFile(String file) {
        return CompressUtil.compressImage(context, Uri.parse(file), maxWidth, maxHeight,
                compressFormat, bitmapConfig, quality, destinationDirectoryPath,
                fileNamePrefix, fileName);
    }
    ...
    public Bitmap compressToBitmap(File file) {
        return CompressUtil.getScaledBitmap(context, Uri.fromFile(file), maxWidth, maxHeight, bitmapConfig);
    }
     ...
     * @return      压缩后的Bitmap
     */
    public Bitmap compressToBitmap(String file) {
        return CompressUtil.getScaledBitmap(context, Uri.parse(file), maxWidth, maxHeight, bitmapConfig);
    }
